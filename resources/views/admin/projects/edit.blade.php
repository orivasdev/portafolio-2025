...
@section('page-description', 'Modifica la información del proyecto')
        ...
        <div class="figma-card p-4 lg:p-6">
            <div class="flex items-center space-x-3 lg:space-x-4">
                <div class="w-10 h-10 lg:w-12 lg:h-12 bg-blue-100 rounded-xl flex items-center justify-center">
                    <i data-feather="edit-2" class="h-5 w-5 lg:h-6 lg:w-6 text-blue-600"></i>
                </div>
                <div>
                    <h2 class="text-lg lg:text-xl font-bold text-gray-900">Editar Proyecto</h2>
                    <p class="text-sm lg:text-base text-gray-600">{{ $project->title }}</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <div>
                    ...
                    <select name="order" 
                        ...
                        @foreach($availableOrders as $orderValue)
                            <option value="{{ $orderValue }}" {{ old('order', $project->order) == $orderValue ? 'selected' : '' }}>
                                Posición {{ $orderValue }}
                                @if($orderValue == $project->order)
                                    (Posición actual)
                                @elseif($orderValue == 1)
                                    (Al inicio)
                                @elseif($orderValue == count($availableOrders))
                                    (Al final)
                                @endif
                            </option>
                        @endforeach
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                        Los demás proyectos se reordenarán automáticamente al cambiar la posición
                    </p>
                    @error('order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-between text-sm text-gray-500">
                        ...
                        <span id="short-desc-count">{{ strlen(old('short_description', $project->short_description)) }}/500</span>
                    </div>
            ...
            <div class="space-y-4">
                ...
                @if($project->image)
                    <div class="mb-4">
                        ...
                        <div class="relative inline-block">
                            <img src="{{ $project->image_url }}" alt="{{ $project->title }}" class="h-32 w-full object-cover rounded-lg border border-gray-200">
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $project->image ? 'Cambiar Imagen' : 'Imagen Principal' }}
                    </label>
            ...
            <div>
                ...
                <div id="technologies-container" class="space-y-2">
                    @if(old('technologies'))
                        ...
                        @endforeach
                    @else
                        @foreach($project->technologies_list as $index => $tech)
                            <div class="flex items-center space-x-2">
                                <input type="text" 
                                       name="technologies[]" 
                                       class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('technologies') border-red-500 @enderror"
...
function updateOrderInfo() {
    ...
    if (selectedOrder === currentOrder) {
        infoElement.innerHTML = '📌 El proyecto mantendrá su posición actual';
    } else if (selectedOrder === 1) {
        infoElement.innerHTML = '📌 El proyecto se moverá al inicio de la lista';
    } else if (selectedOrder === totalProjects) {
        infoElement.innerHTML = '📌 El proyecto se moverá al final de la lista';
    } else if (selectedOrder < currentOrder) {
        infoElement.innerHTML = `📌 El proyecto se moverá a la posición ${selectedOrder}, desplazando los proyectos ${selectedOrder} a ${currentOrder - 1} hacia abajo`;
    } else {
        infoElement.innerHTML = `📌 El proyecto se moverá a la posición ${selectedOrder}, desplazando los proyectos ${currentOrder + 1} a ${selectedOrder} hacia arriba`;
    }
}
